...
public class DijkstraGraphMap {
  ...
  /**
   * Runs dijkstra algorithm using a specified source vertex. Every time when the starting vertex
   * get changed, this algorithm should get run.
   * 
  ...
  /**
   * Get the shortest path as a list of Vertex for a specific destination Vertex with name as
   * endName
   * 
  ...
  public List<Vertex> getShortestPath(final String endName) {
    if (!graphMap.containsKey(endName)) {
      throw new DijkstraException("Graph doesn't contain end vertex : " + endName);
    }
  ...
  private void dijkstra(final NavigableSet<Vertex> que) {
    ...
    while (!que.isEmpty()) {
      ...
      source = que.pollFirst(); // vertex with shortest distance (first iteration will return
                                // source)
